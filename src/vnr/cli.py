...
def doctor() -> None:
    ...
    cuda = prof.cuda_version or ("unavailable" if prof.torch_version else "torch not installed")
    click.echo("VNR SYSTEM DIAGNOSTICS")
    click.echo(f"CPU ..................... {prof.cpu_name} ({prof.cpu_threads} threads)")
    click.echo(f"RAM ..................... {format_bytes(prof.ram_bytes)}")
    click.echo(f"NVIDIA GPU .............. {gpu}")
    click.echo(f"VRAM .................... {vram}")
    click.echo(f"CUDA .................... {cuda}")
    click.echo(f"PyTorch ................. {prof.torch_version or 'missing (optional)'}")
...
def _stub(name: str) -> None:
    phase = _NOT_YET[name]
    click.echo(f"'vnr {name}' is not implemented until {phase} (see 01-master-plan.md)", err=True)
    raise SystemExit(2)
