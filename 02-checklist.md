...
- [ ] `PH4-WI02` FlyWire: `dataset auth`, resumable download, normalized schema, `connectome_report.json/html`; resolve 50M-vs-54.5M in manifest (**UNBLOCKED 2026-09-14 — the block was a wrong conclusion, not a permission**. Access probe: only `flywire_fafb_public` exists/open (others 400, not 403); `synapses_nt_v1` + `proofread_neurons` readable. OFFSET paging measured DEAD (117x decay, 1774h extrapolated). **ID-paging works**: `filter_in_dict(pre_pt_root_id)` = 0.222s/neuron, offset-independent; 139,255 neurons enumerated (matches published count). `bulk_by_id.py` built: 257,428 rows/200 neurons at 12,445 rows/s = 300x OFFSET; resumable (verified across a mid-flight kill); honours Retry-After (429 at 12 workers → default 4). Full-corpus fetch RUNNING. See notes entry 41.)
...
- [x] `PH4-WI03b` Census scaling (DONE 2026-09-14: `census_fast` structural dedupe + 64-entry canonical table; real 1M-edge pilot 1888.1s -> 68.7s (27.5x), per-code counts identical; **reference subset bug found + fixed** (`nodes=` leaked outside neighbors, produced negative null counts); 42 new equivalence tests, full suite 203 green — see notes entry 38)
...
- [x] `PH3-WI04` GPU path (**UNBLOCKED 2026-09-14 — R1 OVERTURNED**: the "cu121 retired / no CUDA for sm_52" verdict was WRONG. cu118 cp313 Windows wheels exist; `torch 2.7.1+cu118` installs and runs on the Quadro M5000 — `cuda.is_available()=True`, capability (5,2), float64 matmul error 6.5e-13, 1M-neuron LIF update OK. VNR's own torch backend is now **bit-exact vs the oracle on device="cuda"**. Honest caveat: bench_gpu measures CUDA 18.875s vs numpy-CPU 0.641s — correct but **29x slower**, because `run_torch` routes every event through Python. GPU is unblocked but not yet useful; kernel-resident event processing is the measured target. See notes entry 41.)
